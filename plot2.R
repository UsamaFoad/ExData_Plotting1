...
data_set <-
    subset(Full_Data, as.Date(Full_Data$Date, "%d/%m/%Y") >= as.Date("01/02/2007",
           "%d/%m/%Y") & as.Date(Full_Data$Date, "%d/%m/%Y") <= as.Date("02/02/2007",
                                                                        "%d/%m/%Y"))
# Add new column to the data with Timestamp (Date & Time)
data_set$Timestamp <- strptime(paste(data_set$Date, data_set$Time), "%d/%m/%Y %H:%M:%S")
...
plot(data_set$Timestamp, data_set$Global_active_power, type = "l", xlab = "", 
